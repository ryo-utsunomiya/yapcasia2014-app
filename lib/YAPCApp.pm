...
use YAPCApp::Container;
use YAPCApp::Renderer;
use YAPCApp::L10N;
use Plack::Session;
...
sub setup_routes {
    ...
    push @{$r->namespaces}, 'YAPCApp::Controller';

    $r->route('/')->to(cb => sub {
        shift->redirect_to("/2014/");
    });

    #notices
    $r->route("/2014/notices/:action")->to(controller => 'notices');
    ...
    $r->get('/2014/api/talk/list')->to('talk#api_list');
    $r->get('/2014/api/talk/show/:object_id')->to('talk#api_show');
    $r->get('/2014/api/timeline/')->to('info#api_timeline');
}
...
sub setup_xslate {
    ...
    $renderer->add_handler(tx => YAPCApp::Renderer->build(
        app => $self,
        syntax => "TTerse",
...
{
    my %langs = (
        'ja' => YAPCApp::L10N->get_handle('ja'),
        'en' => YAPCApp::L10N->get_handle('en'),
    );
    sub loc {
...
sub get_event_description {
    ...
    if ($truncated) {
        $html .= qq|\n<p><a href="/2014/event/show/$event->{id}">read more...</a></p>|;
    }
    return $html;
}
...
sub get_talk_abstract{
    ...
    if ($truncated) {
        $html .= qq|\n<p><a href="/2014/talk/show/$talk->{id}">read more...</a></p>|;
    }
    return $html;
}
